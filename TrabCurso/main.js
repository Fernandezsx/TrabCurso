...
const {
    ...
    ProductManager,
    LogManager,
} = require("./modules/SystemManagers");
...
app.post("/api/update-user", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "ERRO",
            `Erro ao atualizar nome do usuário: ${error.message}`,
        );
...
app.post("/api/admin/login", (req, res) => {
    try {
        ...
        if (user) {
            ...
            LogManager.addLog(
                "INFO",
                `Login administrativo realizado por ${user.usuarioAdm} (${user.role})`,
            );
            ...
            LogManager.addLog(
                "AVISO",
                `Tentativa de login administrativo falhada para usuário: ${username}`,
            );
            res.status(401).json({
                success: false,
        ...
        LogManager.addLog(
            "ERRO",
            `Erro no login administrativo: ${error.message}`,
        );
...
app.get("/api/obj-data", (req, res) => {
    try {
        ...
        LogManager.addLog(
            error,
            `erro ao obter os dados do obj.js: ${error.message}`,
        );
        res.status(500).json({
            success: false,
...
app.get("/api/obj-prod", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "ERRO",
            `Erro ao obter estrutura da classe produto: ${error.message}`,
        );
...
app.post("/api/obj-prod", (req, res) => {
    ...
    try {
        ...
        fs.readFile(filePath, "utf8", (err, data) => {
            ...
            if (!err && data.trim()) {
                try {
                    ...
                    console.log(
                        `📖 Arquivo JSON lido - ${produtos.length} produtos existentes`,
                    );
                } catch (parseError) {
                    console.error(
                        "❌ Erro ao fazer parse do JSON existente:",
                        parseError,
                    );
                    LogManager.addLog(
                        "ERRO",
            ...
            if (!novoProduto.idprod) {
                novoProduto.idprod =
                    produtos.length > 0
                        ...
                        : 1;
                console.log(
                    `🔢 ID gerado automaticamente: ${novoProduto.idprod}`,
                );
            }
            ...
            console.log(
                `➕ Produto adicionado - Total: ${produtos.length} produtos`,
            );
            ...
            fs.writeFile(
                ...
                (writeErr) => {
                    if (writeErr) {
                        console.error(
                            "❌ Erro ao salvar produto no arquivo:",
                            writeErr,
                        );
                        LogManager.addLog(
                            "ERRO",
                            `Erro ao salvar produto: ${writeErr.message}`,
                        );
                    ...
                    // Sucesso!
                    console.log(
                        "✅ Produto salvo com sucesso no arquivo JSON!",
                    );
                    console.log(
                        `📊 Produto registrado: ID ${novoProduto.idprod} - ${novoProduto.nomeProd}`,
                    );

                    LogManager.addLog(
                        "INFO",
                        `Novo produto criado: ID ${novoProduto.idprod} - ${novoProduto.nomeProd}`,
                    );

                    res.json({
                        success: true,
                        message:
                            "Produto registrado com sucesso no arquivo JSON!",
                        produto: novoProduto,
        ...
        LogManager.addLog(
            "ERRO",
            `Erro geral ao criar produto: ${error.message}`,
        );
    ...
    try {
        ...
        fs.readFile(filePath, "utf8", (err, data) => {
            if (err) {
                if (err.code === "ENOENT") {
                    console.log(
                        "📄 Arquivo de produtos não encontrado - retornando lista vazia",
                    );
                    return res.json({
                        success: true,
                ...
                LogManager.addLog(
                    "ERRO",
                    `Erro ao ler produtos salvos: ${err.message}`,
                );
            ...
            try {
                ...
                console.log(
                    `✅ Lista de produtos obtida - ${produtos.length} produtos encontrados`,
                );
                ...
                console.error(
                    "❌ Erro ao fazer parse dos produtos salvos:",
                    parseError,
                );
                LogManager.addLog(
                    "ERRO",
                    `Erro ao fazer parse dos produtos: ${parseError.message}`,
                );
        ...
        LogManager.addLog(
            "ERRO",
            `Erro geral ao listar produtos: ${error.message}`,
        );
...
app.get("/api/admin/dashboard", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "ERRO",
            `Erro ao obter dados do dashboard: ${error.message}`,
        );
...
app.post("/api/admin/products", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "INFO",
            `Novo produto adicionado: ${newProduct.nome}`,
        );
        res.json(newProduct);
    } catch (error) {
...
app.put("/api/admin/products/:id", (req, res) => {
    try {
        ...
        if (updatedProduct) {
            LogManager.addLog(
                "INFO",
                `Produto atualizado: ${updatedProduct.nome}`,
            );
...
app.get("/api/admin/system-events", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "ERRO",
            `Erro ao obter eventos do sistema: ${error.message}`,
        );
...
app.post("/api/admin/logout", (req, res) => {
    try {
        ...
        LogManager.addLog(
            "INFO",
            `Logout administrativo realizado por ${username || "usuário desconhecido"}`,
        );
        res.json({
            success: true,
...
app.listen(PORT, () => {
    ...
    LogManager.addLog(
        "INFO",
        `Servidor Café Quente iniciado na porta ${PORT} com painel administrativo integrado`,
    );
});
